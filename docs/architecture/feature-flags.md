...
- `FeatureFlagKey`
    - Takes the key in as a string
    - Only currently has 2 feature flags in the Keys object,
...
- `DefaultFeatureFlagOverrides`
    ...
    - Has one function, `getCatalog()` which returns a `Flow<List<FeatureFlag>>`

- `FeatureFlagFactory`
    - Interface that has to be implemented in each application/module
    - K9: `single<FeatureFlagFactory> { K9FeatureFlagFactory() }`
        - `app.k9mail` package, `appModule` defines it in the `K9KoinModule` file
    - Thunderbird: `single<FeatureFlagFactory> { TbFeatureFlagFactory() }`
        - Located in `net.thunderbird.android` package, in the `appModule` definition, `ThunderbirdKoinModule.kt`
...
- `TbFeatureFlagFactory`
    ...
    - Different versions in different directories to define the build. Each build will use the `TbFeatureFlagFactory` from its own directory. See the "[How to Add a Feature Flag](#how-to-add-a-feature-flag)" section below to find all of the factory locations
    ...
    - You can set up feature flags in specific classes, but they still have to be named individually here, for example, these are feature flags related to MessageList:
        ...
        - These are both defined in `MessageListFeatureFlags` but we don’t have a way to pass in all of them currently, so we have to list each one individually
